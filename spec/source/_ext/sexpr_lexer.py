...
class SExprLexer(RegexLexer):
    ...
    tokens = {
        'root' : [
            ...
            (r'\b((assert|cover|assume|restrict)\-property)|((cover|trigger)\-sequence)\b', Keyword),
            (r'\b((assert|cover|assume|restrict)\-simple\-property)|((cover|trigger)\-simple\-sequence)\b', Keyword),
            (r'\;.*', Comment),
            (r'(<[a-zA-Z0-9_\-]+>)', String),
            (r'\b([0-9]+|true|false|range|bounded\-range)\b', Number),
            (r'\$', Number),
            (r'\#[a-z\-]+', String),
            (r'\b(not|or|and|constant|initial|eq|xor)\b', Name.Tag),
